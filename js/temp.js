function goAdd()
{
	document.location.href="http://localhost/financialSystem/addStudent.html";
}
...
function page(p)
{
	...
	if (p==0)
		if (now>1)
		{
			$("#detail").empty();
			now--;
			$.get("/financialSystem/controller/controller.php?p=8&page="+now,function(data,status)
			{ 
			createData(data); });
...
function skip(now)
{
	if (event.keyCode == 13)
	{
		...
		$("#detail").empty();
		$.get("/financialSystem/controller/controller.php?p=8&page="+target,function(data,status)
		{
			createData(data); 
		});
...
{
	$.get("/financialSystem/controller/controller.php?p=8&page=1",function(data,status)
	{ createData(data); })
	$.get("/financialSystem/controller/controller.php?p=12&table=income",function(data,status)
	{
		setPage(data,1);
	})
...
$(document).ready(
	function()
	{
		$('#delete').click(function()
		{
			...
			$.post("/financialSystem/controller/delete.php",{data:chosedId});
		})
	})